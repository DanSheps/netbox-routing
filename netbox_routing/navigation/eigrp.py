...
routers = PluginMenuItem(
    ...
    buttons=(
        PluginMenuButton('plugins:netbox_routing:eigrprouter_add', 'Add', 'mdi mdi-plus', ColorChoices.GREEN),
        PluginMenuButton('plugins:netbox_routing:eigrprouter_import', 'Import', 'mdi mdi-upload', ColorChoices.CYAN),
    )
)
address_families = PluginMenuItem(
    link='plugins:netbox_routing:eigrpaf_list',
    link_text='Address Families',
    permissions=['netbox_routing.view_eigrpaddressfamily'],
    buttons=(
        PluginMenuButton('plugins:netbox_routing:eigrpaf_add', 'Add', 'mdi mdi-plus', ColorChoices.GREEN),
    )
)
networks = PluginMenuItem(
    ...
    buttons=(
        PluginMenuButton('plugins:netbox_routing:eigrpnetwork_add', 'Add', 'mdi mdi-plus', ColorChoices.GREEN),
    )
)
interfaces = PluginMenuItem(
    ...
    buttons=(
        PluginMenuButton('plugins:netbox_routing:eigrpinterface_add', 'Add', 'mdi mdi-plus', ColorChoices.GREEN),
    )
)
