...
class OSPFInterfaceFilter(InterfaceMixin, VRFMixin, BaseObjectTypeFilterMixin):
    instance: Annotated['OSPFInstanceFilter', strawberry.lazy('netbox_routing.graphql.filters')] | None = (
        strawberry_django.filter_field()
    )
    instance_id: ID | None = strawberry_django.filter_field()
    area: Annotated['OSPFAreaFilter', strawberry.lazy('netbox_routing.graphql.filters')] | None = (
        strawberry_django.filter_field()
    )
...
class EIGRPAddressFamilyFilter(BaseObjectTypeFilterMixin):
    router: Annotated['EIGRPRouterFilter', strawberry.lazy('netbox_routing.graphql.filters')] | None = (
        strawberry_django.filter_field()
    )
...
class EIGRPNetworkFilter(NetworkPrefixMixin, BaseObjectTypeFilterMixin):
    ...
    router_id: ID | None = strawberry_django.filter_field()
    address_family: Annotated['EIGRPAddressFamilyFilter', strawberry.lazy('netbox_routing.graphql.filters')] | None = (
        strawberry_django.filter_field()
    )
